...
function objectWithKey<K extends PropertyKey>(value: unknown, key: K): value is Record<K, unknown> {
  return typeof value === 'object' && value !== null && key in value
}

function isAsyncIterable(value: unknown): value is AsyncIterable<unknown> {
  return (
    objectWithKey(value, Symbol.asyncIterator) && typeof value[Symbol.asyncIterator] === 'function'
  )
}

export function isReadableStreamLike(x: unknown): x is ReadableStream<unknown> {
  return objectWithKey(x, 'getReader') && typeof x.getReader === 'function'
}
...
export async function* normalizeToAsyncStringIterable(
  ...
  const maybeReadable = isReadableStreamLike(resolved)
    ...
    : objectWithKey(resolved, 'readable') && isReadableStreamLike(resolved.readable)
      ? resolved.readable
      : null
